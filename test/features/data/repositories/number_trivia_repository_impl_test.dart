  ...
  group("getRandomNumberTrivia", () {
    ...
    test("Should check if the device is online", () {
      ...
      verify(mockNetworkInfo
          .isConnected); // verify that mockNetworkInfo.isConnected is called
    });
    ...
    group("Device is online", () {
      ...
      test(
        ...
        final result = await repository.getRandomNumberTrivia();
        // assert
        verify(mockRemoteDataSource.getRandomNumberTrivia()); // Check that "mockRemoteDataSource.getRandomNumberTrivia(tNumber)" is called
        verifyZeroInteractions(
            mockLocalDataSource); // Check "mockLocalDataSource" has no interaction such as call method "cacheNumberTrivia" after an exception is happened
        expect(result, Left(ServerFailure())); // Left means Failure at Future<Either<Failure, NumberTrivia>>
      });
    });
